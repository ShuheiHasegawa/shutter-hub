 ...
 *
 * 使用方法:
 * node scripts/create-halymoon-models.js                    # 既存ユーザーを使用（デフォルト・推奨）
 * node scripts/create-halymoon-models.js --create-users     # ユーザーを新規作成（時間がかかります）
 */
...
async function main() {
  ...
  try {
    ...
    for (const modelData of halymoonModelsData) {
    ...
    console.log(
      `\n📝 Malymoon（マリームーン）所属モデル一覧（${halymoonModelsData.length}名）:`
    );
    halymoonModelsData.forEach((model, index) => {

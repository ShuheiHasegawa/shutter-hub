...
#### **現状（リリース前）**
- ✅ 新規実装のみuseSWR使用（`useProfile.ts`, `useActivityCharts.ts`）
- ✅ 既存12ページは現状維持（リスク回避）
- ✅ パフォーマンス監視システムで効果測定

#### **Phase 1: リリース後1-2週間（高優先度）**
```yaml
対象ページ:
  - dashboard/page.tsx（複数データフェッチ、頻繁アクセス）
  - photo-sessions/page.tsx（検索・フィルタ、重いクエリ）
  - studios/page.tsx（検索・フィルタ）
...
#### **Phase 2: リリース後1ヶ月（中優先度）**
```yaml
対象ページ:
  - photo-sessions/[id]/page.tsx（詳細ページ）
  - studios/[id]/page.tsx（詳細ページ）
  - timeline/page.tsx（リアルタイム性重要）
...
#### **移行判断基準**
1. **パフォーマンスダッシュボード**での問題検出
2. **ユーザーフィードバック**での遅延報告
3. **アクセス頻度**と**データ複雑度**
4. **開発工数**と**リスク**のバランス

#### **useSWR導入のメリット**
- ✅ **重複実行防止**: `dedupingInterval`で自動制御
- ✅ **キャッシュ管理**: 手動実装不要、自動最適化
- ✅ **コード量削減**: useState + useEffect → useSWR（50%削減）
- ✅ **バグ削減**: 依存配列の問題を回避

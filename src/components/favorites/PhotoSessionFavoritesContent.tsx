...
import { Heart, Camera, Search, Filter } from 'lucide-react';
...
export function PhotoSessionFavoritesContent() {
  ...
  const fetchFavorites = async () => {
    try {
      setLoading(true);
      const result = await getUserFavoritesAction('photo_session', 1, 50);
      ...
      Logger.error('Failed to fetch photo session favorites:', error);
    ...
    <div className="space-y-6">
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Camera className="h-5 w-5 text-theme-accent" />
              <span className="font-semibold text-theme-text-primary">
                お気に入り撮影会
              </span>
              <Badge variant="secondary" className="font-semibold">
                {photoSessionFavorites.length}件
              </Badge>
            </div>
          </div>
        </CardContent>
      ...
      {filteredItems.length === 0 ? (
        <EmptyFavorites
          ...
          message={
            searchTerm
              ? `「${searchTerm}」に該当する撮影会が見つかりませんでした`
              : 'まだお気に入りの撮影会がありません'
          }

...
export default async function AdminUsersPage() {
  try {
    ...
    const regularUsers =
      allUsers?.filter(
        (u: { role?: string | null }) => u.role === 'user' || !u.role
      ) || [];
    const pendingInvitations = invitations.filter(
      (inv: { used_at?: string | null }) => !inv.used_at
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                ...
                {
                  adminUsers.filter(
                    (u: { role?: string | null }) => u.role === 'super_admin'
                  ).length
                }
                名
              </p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {
                  regularUsers.filter((u: { created_at: string }) => {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {error instanceof Error ? error.message : '管理者権限が必要です'}
          </AlertDescription>
        </Alert>
      </div>
    );
